...
class Controller(BaseHTTPRequestHandler):
    def sync(self, parent: dict, related: dict) -> dict:
        ...
        included_keys: list[str] = parent['spec']['includedKeys']
        target_configmaps = [self.new_configmap(
            sourceName, namespace, original_configmap['data'], included_keys) for namespace in targetNamespaces]
        return target_configmaps

    def new_configmap(self, name: str, namespace: str, data: dict, included: list[str]) -> dict:
        if included:
            new_data = {key: data[key] for key in included}
        else:
            new_data = data
        return {
            'apiVersion': 'v1',
